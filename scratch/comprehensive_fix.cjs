...
// 2. Add missing state
if (!content.includes('isProfileOpen')) {
    content = content.replace('const [isSaving, setIsSaving] = useState(false);', 
        'const [isSaving, setIsSaving] = useState(false);\n  const [isProfileOpen, setIsProfileOpen] = useState(false);');
}

// 3. Fix fetchData queries
if (!content.includes('let registrationsQuery')) {
    content = content.replace("let usersQuery = supabase.from('profiles').select('*');", 
